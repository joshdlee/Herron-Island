...
const TodayFerrySchedule = () => {
  ...
  const applyLowTideModifications = (schedule, date) => {
    ...
    ["mainland", "island"].forEach((location) => {
      if (lowTideInfo.cancel && lowTideInfo.cancel[location]) {
        modifiedSchedule[location].departure_times = modifiedSchedule[
          location
        ].departure_times.filter(
      ...
      if (lowTideInfo.reschedule && lowTideInfo.reschedule[location]) {
        modifiedSchedule[location].departure_times = [
          ...
          const aTime = aTime24
            .split(":")
            .map((x, i) => (i === 0 ? parseInt(x) * 60 : parseInt(x)));
          const bTime = bTime24
            .split(":")
            .map((x, i) => (i === 0 ? parseInt(x) * 60 : parseInt(x)));
      ...
      if (lowTideInfo.reschedule && lowTideInfo.reschedule[location]) {
        modifiedSchedule[location].reschedule =
          lowTideInfo.reschedule[location].departure_times;
      }

      if (lowTideInfo.cancel && lowTideInfo.cancel[location]) {
        modifiedSchedule[location].cancel =
          lowTideInfo.cancel[location].departure_times;
      }
    });
    return modifiedSchedule;
  };
  const renderNote = (location) => {
    const rescheduleItems = schedule[location].reschedule
      ? schedule[location].reschedule.map((time) => (
          <Chip
            key={time}
    ...
    const cancelItems = schedule[location].cancel
      ? schedule[location].cancel.map((time) => (
          <Chip
            key={time}
            textStyle={{ fontSize: 10 }}
            icon={() => (
              <MaterialCommunityIcons name="cancel" size={10} color="white" />
            )}
  ...
  const getTodaySchedule = () => {
    ...
    let schedule = isSummer
      ? ferrySchedule.summer[day]
      : ferrySchedule.winter[day];
  ...
  const islandTableData = removeDuplicates(schedule.island.departure_times).map(
    (time) => [time]
  );
  ...
  const renderScrollIndicator = () => {
    return (
      <View>
        <LinearGradient colors={["rgba(0,0,0,0)", theme.colors.background]} />
