...
export default function GetPrototypeSchedules() {
...
const handleRefresh = async () => {
    setRefreshing(true);
    await fetchPrototypeSchedules();
    setRefreshing(false);
};

  async function fetchPrototypeSchedules() {
    const season = "prototype";
  ...
  const saveDataToLocalState = async (data) => {
    try {
        await AsyncStorage.setItem('prototypeSchedules', JSON.stringify(data));
        await AsyncStorage.setItem('prototypeLastUpdated', Date.now().toString()); // save the timestamp
    } catch (error) {
  ...
  const fetchDataFromLocalState = async () => {
    try {
        const savedData = await AsyncStorage.getItem('prototypeSchedules');
        const lastUpdated = await AsyncStorage.getItem('prototypeLastUpdated');
        ...
        if (savedData !== null && lastUpdated !== null) {
            ...
            if (timeElapsed > oneDay) {
                fetchPrototypeSchedules(); // fetch new data if more than 24 hours old
            } else {
                setSchedules(JSON.parse(savedData));
                setLoading(false);
            }
        } else {
            fetchPrototypeSchedules(); // if no data is found, fetch it
        }
    } catch (error) {
    ...
    <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
      {loading ? ( // If loading is true, show ActivityIndicator
        ...
        <>
          ...
          >
            Prototype Daily Schedules (August 11 - October 5)
          </Text>
