...
export default function GetSummerSchedules() {
...
const handleRefresh = async () => {
    setRefreshing(true);
    await fetchSummerSchedules();
    setRefreshing(false);
};

  async function fetchSummerSchedules() {
    const season = "summer";
  ...
  const saveDataToLocalState = async (data) => {
    try {
        await AsyncStorage.setItem('summerSchedules', JSON.stringify(data));
    ...
    try {
        ...
        if (savedData !== null && lastUpdated !== null) {
            ...
            if (timeElapsed > oneDay) {
                fetchSummerSchedules(); // fetch new data if more than 24 hours old
            } else {
                setSchedules(JSON.parse(savedData));
                setLoading(false);
            }
        } else {
            fetchSummerSchedules(); // if no data is found, fetch it
        }
    } catch (error) {
